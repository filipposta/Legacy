...
import { 
  ...
  getDocs, query, where, arrayRemove, setDoc, 
  writeBatch, arrayUnion, addDoc, serverTimestamp
...
const ChatList: React.FC<ChatListProps> = ({ onClose, isLoading = false }) => {
  ...
  const [friendsInfo, setFriendsInfo] = useState<Record<string, any>>({})
  ...
  const handleDeleteChat = async (chat: any, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleLeaveGroup = async (chat: any, e: React.MouseEvent) => {
    e.stopPropagation();
    ...
    try {
      ...
      if (isAdmin) {
        const otherMembers = membersList.filter((id: string) => id !== currentUserId);
  ...
  const handleDeleteChatFromList = async (chat: any, event: React.MouseEvent) => {
    ...
    try {
      if (chat.isGroup) {
        ...
        if (chatDoc.exists()) {
          ...
          const updatedParticipants = chatData.participants.filter((id: string) => id !== user?.id)
  ...
  const filteredChats = chats.filter(chat => {
    ...
    if (chat.isGroup) {
      ...
      return (
        chat.otherUser?.displayName?.toLowerCase().includes(searchLower) ||
    ...
    if (filteredFriends.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center h-full text-slate-400 px-4">
          <UserIcon className="w-8 h-8 sm:w-10 sm:h-10 mb-2 sm:mb-3 opacity-50" />
          <p className="text-center text-xs sm:text-sm">
            {searchTerm ? 'No friends found' : 'No friends yet. Add some friends!'}
          </p>
        </div>
      )
    }
    ...
    return (
      filteredFriends.map((friendId) => {
        ...
        return (
          <div
            key={friendId}
            onClick={() => !isBlockedByCurrentUser && !loading && handleStartChat(friendId)}
          ...
          >
            <div className="flex items-center space-x-2 sm:space-x-3">
              <div className="flex-shrink-0 relative">
                {info?.avatar ? (
                  <img src={info.avatar} alt="Friend" className="w-8 h-8 sm:w-10 sm:h-10 rounded-full object-cover" />
                ) : (
                  <div className="w-8 h-8 sm:w-10 sm:h-10 bg-blue-600 rounded-full flex items-center justify-center">
                    <span className="text-white font-semibold text-xs">
                      {(info?.name || friendId).charAt(0).toUpperCase()
                    }</span>
                  </div>
                )}
                {(loading || isLoading) && handleStartChat && info && (
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs sm:text-sm text-slate-400">
                  {isBlockedByCurrentUser ? 'Blocked' : (loading || isLoading) ? 'Loading...' : 'Tap to chat'}
                </p>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="flex flex-col h-full bg-black">
      ...
      <div className="p-3 border-b border-gray-800 flex-shrink-0">
        ...
        <div className="mt-3 relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-gray-900 border-none rounded-full text-white placeholder-gray-500 focus:outline-none text-sm"
          />
        </div>
        
        {/* Tab buttons with X style */}
      ...
      <div className="flex-1 overflow-auto">
          ...
          <div className="py-2 px-3 space-y-1">
            {filteredChats.length === 0 ? (
              ...
              filteredChats.map((chat) => (
                <div
                  key={chat.id}
                  className="relative group p-3 rounded-lg cursor-pointer transition-all hover:bg-gray-900/50"
                  onClick={() => setSelectedChat(chat)}
                >
                  <div className="flex items-center">
                    {/* Make the avatar/logo much bigger */}
                    <div className="w-20 h-20 rounded-full bg-gray-800 flex-shrink-0 mr-3 flex items-center justify-center">
                      {(chat.isGroup && chat.groupPhoto) ? (
                        <img src={chat.groupPhoto} alt="Group" className="w-full h-full rounded-full object-cover" />
                      ) : chat.isGroup ? (
                        <span className="text-white font-bold text-3xl">
                          {(chat.groupName || 'G').charAt(0).toUpperCase()}
                        </span>
                      ) : chat.otherUser?.profilePic ? (
                        <img src={chat.otherUser.profilePic} alt="User" className="w-full h-full rounded-full object-cover" />
                      ) : (
                        <span className="text-white font-bold text-3xl">
                          {(chat.otherUser?.displayName || chat.otherUser?.username || 'U').charAt(0).toUpperCase()}
                        </span>
                      )}
                    </div>
                      ...
                      <div className="flex justify-between">
                        <h3 className="font-medium text-white truncate">
                          {chat.isGroup ? 
                            ...
                            (chat.otherUser?.displayName || chat.otherUser?.username || 'Unknown User')
                          }
                        </h3>
                        {chat.lastMessageTime && (
                          <span className="text-xs text-gray-500">
                            {new Date(chat.lastMessageTime.toDate?.() || chat.lastMessageTime).toLocaleDateString([], { 
                              month: 'short', 
                              day: 'numeric' 
                            })}
                          </span>
                  ...
                  {/* Action Buttons - Positioned properly and displayed on both mobile/desktop */}
                  <div className="absolute right-2 top-1/2 transform -translate-y-1/2 flex items-center space-x-1">
                    {/* Leave Group Button - Only for groups */}
                    {chat.isGroup && (
                      <button
                        onClick={(e) => handleLeaveGroup(chat, e)}
                        className="p-1.5 bg-yellow-500/80 hover:bg-yellow-500 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity md:opacity-0 md:group-hover:opacity-100"
                        title={chat.adminId === user?.uid ? "Leave/Transfer Group" : "Leave Group"}
                      >
                    ...
                    <button
                      ...
                      className="p-1.5 bg-red-500/80 hover:bg-red-500 text-white rounded-full opacity-70 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity"
                      title="Delete Chat"
                    >
        ...
        {tab === 'friends' && (
          <div className="h-full overflow-y-auto p-1.5 sm:p-2 lg:p-3 space-y-1.5 sm:space-y-2">
            {renderFriendsList()}
          </div>
        )}
      </div>
      ...
      {showTransferAdminModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[200] p-4">
          <div className="bg-gray-900 rounded-xl p-5 max-w-md w-full">
            <h3 className="text-lg font-bold text-white mb-4">Transfer Admin Rights</h3>
            <p className="text-gray-300 mb-4">
              You're the group admin. Choose a new admin or delete the group:
            </p>
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto mb-4">
              {groupMembers.map(member => (
                <button
                  key={member.id}
                  onClick={() => handleTransferAdminAndLeave(member.id)}
                  className="w-full text-left p-3 rounded-lg bg-gray-800 hover:bg-blue-800 text-white transition-colors"
                >
                  Make {member.name} the new admin
                </button>
              ))}
            </div>
