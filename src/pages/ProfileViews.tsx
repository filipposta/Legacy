...
const ProfileViews = () => {
  ...
  const [viewData, setViewData] = useState<ViewerData[]>([]);
  ...
  const fetchProfileViews = async (userId: string) => {
    setError(null);
    try {
      ...
      const viewerMap = new Map<string, ViewerData>();
      ...
      if (invalidViewIds.length > 0) {
        ...
        const deletePromises = invalidViewIds.map(id => 
          deleteDoc(doc(db, "profileViews", id)).catch(err => 
            console.error("Error deleting invalid view:", err)
          )
        );
      ...
      const views = Array.from(viewerMap.values()).sort((a, b) => 
  ...
  const cleanInvalidViews = async () => {
    ...
    try {
      ...
      const seenViewers = new Map<string, { id: string, timestamp: Date }>();
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl bg-gray-900 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-white">Profile Viewers</h1>
        <button
          onClick={cleanInvalidViews}
          disabled={cleaning || viewData.length === 0}
          className="flex items-center space-x-2 bg-red-600 hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-4 py-2 rounded-lg transition-colors duration-200"
          title="Remove invalid profile view entries"
        >
          {cleaning ? (
            <>
      ...
      {error && (
        <div className="bg-yellow-900/30 border-l-4 border-yellow-400 p-4 mb-6 flex items-start">
          <ExclamationTriangleIcon className="h-6 w-6 text-yellow-400 mr-3" />
          <div>
            <p className="font-medium text-yellow-200">Unable to access profile view data</p>
      ...
      {viewData.length > 0 ? (
        <div className="bg-gray-800 rounded-lg shadow overflow-hidden border border-gray-700">
          <div className="p-4 border-b border-gray-700">
            <h2 className="text-lg font-semibold text-white">Recent Profile Viewers</h2>
            <p className="text-sm text-gray-400 mt-1">
              {viewData.length} unique viewer{viewData.length !== 1 ? 's' : ''} found
            </p>
          </div>
          <ul className="divide-y divide-gray-700">
            {viewData.map((view) => (
              <li key={view.id} className="p-4 hover:bg-gray-750 flex items-center">
                <div className="flex-shrink-0 h-12 w-12 rounded-full overflow-hidden bg-gray-600 relative">
                  {view.viewer.profilePic ? (
                    <img 
                      src={view.viewer.profilePic} 
                    ...
                    <div className="h-full w-full flex items-center justify-center bg-gradient-to-r from-purple-500 to-pink-500">
                      <span className="text-white font-bold text-lg">
                        {(view.viewer.displayName || view.viewer.username || 'U').substring(0, 2).toUpperCase()}
                      </span>
                    </div>
                  )}
                </div>
                <div className="ml-4 flex-1">
                  <div className="flex items-center justify-between">
                    <p className="font-medium text-white">{view.viewer.displayName}</p>
                    <div className="flex items-center text-sm text-gray-400">
                      <ClockIcon className="h-4 w-4 mr-1" />
                      {view.viewedAt instanceof Date && !isNaN(view.viewedAt.getTime())
        ...
        <div className="bg-gray-800 rounded-lg shadow overflow-hidden border border-gray-700 p-4 text-center">
          <p className="text-gray-400">No profile views found.</p>
        </div>
      )}
    </div>
  );
