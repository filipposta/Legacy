 ...
 */
export function safeOnSnapshot(query, observerOrNext, error, complete) {
    // Handle different call signatures
    const isObserverObject = typeof observerOrNext === 'object';
    // Prepare error handler
    const safeErrorHandler = (err) => {
    ...
    try {
        if (isObserverObject) {
            ...
            // Case: onSnapshot(query, next, error, complete)
            return onSnapshot(query, observerOrNext, safeErrorHandler, complete);
        }
    }
    catch (err) {
